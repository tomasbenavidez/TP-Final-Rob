...
#   opcional, otra ruta:  map_output:=/ruta/mi_mapa
...
ros2 launch tp_b_navigation parte_b.launch.py \
    map_yaml:=$HOME/Documents/GitHub/TP-Final-Rob/mapas/map_sim.yaml
```
...
- `map_loader.py` y `parte_b.launch.py`: **default = `map_sim.yaml`** (el del profe queda como
  fallback vía `map_yaml:=.../map.yaml`).
